...
object SeqArrays extends SeqArrays with SeqTreeImplementation
import SeqArrays._
...
//TODO implement Zipped wrapper for convenience functions
class PArraysTest {

  @Test def stdArrayOperations = {
    val arr = Array(1,2,3)
    val parr = fromArray(arr)
    assertThat("equal length", parr.length, is(3))
  }
  ...
  @Test def splits = {
    ...
    // PA[A]
    val nested = tabulate(3)(i => arr.map {j => (arr.length*i) + j+1})
  ...
  @Test def dslSamples = {
    ...
    import scalan.util.{ArrayFormatting => PA}
    import scalan.samples._
    object PSamples extends DslSamples with ScalanSequential
    import PSamples._
    ...
    val rowinds = fromArray(0 to 1)
    val rowvals = replicate(2, 2.3f)
    val row = rowinds zip(rowvals)
    val matr = replicate(2,row)
    val vec = replicate(2, 0.5f)
    val res = PSamples.matrixVectorMul(matr, vec)
